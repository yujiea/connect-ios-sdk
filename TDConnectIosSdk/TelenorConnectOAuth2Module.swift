...
public class TelenorConnectOAuth2Module: OAuth2Module {
    ...
    private func logOut(completionHandler: @escaping (AnyObject?, NSError?) -> Void) {
        ...
        if (self.oauth2Session.tokenIsNotExpired()) {
            callLogOutEndpoint(completionHandler: { (_, _) in
                self.revokeAndClearTokens(completionHandler: completionHandler)
            })
            return
        }

        self.refreshAccessToken(completionHandler: { (_, errorRefresh) in
            if (errorRefresh != nil) {
                self.revokeAndClearTokens(completionHandler: completionHandler)
                return
            }

            self.callLogOutEndpoint(completionHandler: { (_, _) in
                self.revokeAndClearTokens(completionHandler: completionHandler)
            })
        })
    }
    ...
    private func revokeAccessToken() {
        ...
        http.request(method: .post, path: config.revokeTokenEndpoint!, parameters: paramDict, responseSerializer: StringResponseSerializer(), completionHandler: { (_, _) in
        })
    }
